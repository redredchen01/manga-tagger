"""Test the complete optimized flow with English to Chinese mapping."""
...
print("=" * 70)
print("Test Complete Optimized Flow with Mapping")
print("=" * 70)

# Test tag mapper
print("\n[0] Testing Tag Mapper")
mapper = get_tag_mapper()
test_words = ["loli", "catgirl", "school uniform", "demon"]
for word in test_words:
    cn = mapper.to_chinese(word)
    print(f"    '{word}' -> '{cn}'")
...
# Test tag matching with mapping
print("\n[3] Testing tag library matching with mapping...")
...
for kw in keywords:
    ...
    if cn:
        mapped_keywords.append(cn)
        print(f"    Mapped: '{kw}' -> '{cn}'")
    else:
...
if mapped_keywords:
    matches = tag_lib.match_tags_by_keywords(mapped_keywords, min_confidence=0.5)
