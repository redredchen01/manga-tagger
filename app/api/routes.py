...
async def tag_cover(
    ...
    try:
        ...
        # Fallback: Add tags from VLM results using enhanced tag matching
        if not tags and vlm_metadata:
            try:
                # Use our enhanced tag_matcher for VLM description
                from tag_matcher import create_tag_matcher

                matcher = create_tag_matcher()

                # Extract description from VLM
                if hasattr(vlm_metadata, "description") and vlm_metadata.description:
                    description = vlm_metadata.description

                    # Use tag_matcher to find matching tags
                    matcher_results = matcher.match(
                        description,
                        top_k=top_k,
                    ...
                    for result in matcher_results:
                        ...
                        if not any(t.tag == result.tag_name for t in tags):
                            tags.append(
                                TagResult(
                                    tag=result.tag_name,
                                    confidence=safe_confidence(result.similarity),
                                    source="vlm+hybrid",
                                    reason=f"Matched from VLM description: {result.matched_text[:50]}...",
                                )
                            )

                    logger.info(
                        f"Enhanced matching found {len(matcher_results)} tags from VLM description"
                    )

            except Exception as e:
                logger.warning(
                    f"Enhanced tag matching failed: {e}, falling back to basic VLM"
                )
