...
class ChineseEmbeddingService:
    ...
    async def encode_batch(self, texts: List[str]) -> np.ndarray:
        ...
        try:
            ...
            embeddings = await loop.run_in_executor(self.executor, self._encode_texts_sync, texts)
            return embeddings
    ...
    def _generate_fallback_embeddings(self, texts: List[str]) -> np.ndarray:
        ...
        for text in texts:
            ...
            embedding = rng.standard_normal(self.target_dim, dtype=np.float32)
    ...
    async def search_tags_by_text(
        ...
        try:
            ...
            for result in results:
                tag_results.append({"tag": result["text"], "similarity": result["similarity"]})
            return tag_results
    ...
    async def search_cached_tags(
        ...
        try:
            ...
            for idx in top_indices:
                ...
                if score >= threshold:
                    results.append({"tag": self._tag_names_cache[idx], "similarity": score})
            return results
