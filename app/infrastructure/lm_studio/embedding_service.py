...
class LMStudioEmbeddingService:
    ...
    def __init__(self):
        ...
        logger.info("LM Studio embedding service initialized")
    ...
    async def generate_embedding(self, image_bytes: bytes, target_dim: int = 4096) -> List[float]:
        """
    ...
    async def _try_lm_studio_embedding(self, image_bytes: bytes) -> Optional[List[float]]:
        ...
        try:
            ...
            # Fallback: use vision model to get features while the client is still open.
            return await self._extract_features_via_vision(base64_image, headers, client)
    ...
    def _adjust_dimension(self, embedding: List[float], target_dim: int = 4096) -> List[float]:
    ...
    def _text_to_embedding(self, text: str, target_dim: int = 4096) -> List[float]:
        ...
        embedding = rng.standard_normal(target_dim, dtype=np.float32)
